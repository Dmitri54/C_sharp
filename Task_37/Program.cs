...
int[] MutArrEl (int[] array) // Метод - перемножает первый и последний элемент, второй и предпоследний и т.д.
{
    ...
    for (int i = 0; i < ten; i++)
    {
        ResArr[i] = array[i] * array[array.Length - 1 - i]; //-1 чтобы не было выхода за диапозон массива, т.к. счет идет отчет от 0. 
    }
    if (array.Length % 2 != 0) ResArr[ten - 1] = array[ten - 1]; //  Нужно определить, если массив был изначально
